...
export default function OptionsPage() {
  ...
  const canCallApi = useMemo(() => norm(backendUrl).length > 0 && norm(apiKey).length > 0, [backendUrl, apiKey])
  ...
  useEffect(() => {
    ;(async () => {
      ...
      setBackendUrl(items.backendUrl || DEFAULT_BACKEND_URL)
  ...
  const saveBackend = async () => {
    setError(null)
    setNotice(null)
    await storageSet({ backendUrl: norm(backendUrl), apiKey: norm(apiKey) })
    setNotice("Saved.")
  }
  ...
  const connectGmail = async () => {
    setError(null)
    setNotice(null)
    const base = norm(backendUrl) || DEFAULT_BACKEND_URL
  ...
  const refresh = async () => {
    ...
    try {
      const [settings, jobList] = await Promise.all([
        apiFetch<BackendSettings>(backendUrl, apiKey, "/settings"),
        apiFetch<{ jobs: Job[] }>(backendUrl, apiKey, "/jobs"),
      ])
  ...
  const updateCancelRule = async (next: CancelRule) => {
    ...
    setNotice(null)
    try {
      await apiFetch<BackendSettings>(backendUrl, apiKey, "/settings", {
        method: "PUT",
  ...
  const cancelJob = async (id: string) => {
    ...
    setNotice(null)
    try {
      await apiFetch(backendUrl, apiKey, `/jobs/${encodeURIComponent(id)}/cancel`, { method: "POST" })
      await refresh()
      setNotice("Cancelled.")
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e))
  ...
  return (
    <div style={{ fontFamily: "system-ui, Segoe UI, Roboto, Arial, sans-serif", padding: 16, maxWidth: 820 }}>
      ...
      <div style={{ display: "grid", gap: 10, border: "1px solid rgba(0,0,0,.12)", borderRadius: 12, padding: 12 }}>
        <div style={{ display: "grid", gap: 6 }}>
          <label style={{ fontWeight: 700 }}>Backend URL</label>
          <input
            value={backendUrl}
            ...
            {jobs.length === 0 ? (
              <tr>
                <td colSpan={6} style={{ padding: 12, color: "rgba(0,0,0,.65)" }}>
                  {canCallApi ? "No jobs." : "Enter Backend URL + API key, then Refresh."}
                </td>
              </tr>
            ) : (
              jobs.map((j) => (
